<script lang="ts">
	...
	const resolved = PUBLIC_COMMIT_SHA && !PUBLIC_COMMIT_SHA.startsWith('$');
	const shortSha = resolved ? PUBLIC_COMMIT_SHA.substring(0, 7) : 'dev';
	const commitLinkUrl = resolved ? `${Site.repo.commitBaseUrl}${PUBLIC_COMMIT_SHA}` : '#';
