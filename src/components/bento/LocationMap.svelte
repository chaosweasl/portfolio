<script lang="ts">
	...
	function updateTime() {
		const now = new Date();
		const enschedeTime = new Intl.DateTimeFormat('en-US', {
		...
		currentTime = enschedeTime;

		// Check if daytime (6 AM - 9 PM)
		const hour = parseInt(enschedeTime.split(':')[0]);
	...
	function recenterMap() {
		if (mapInstance) {
			mapInstance.setView([52.2215, 6.8937], 12);
		}
	}
</script>
